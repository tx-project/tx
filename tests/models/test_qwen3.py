...
from xtrain.models import Qwen3Model
...
def load_checkpoint(filename: str | os.PathLike, model: nnx.Module) -> None:
    ...
    def get_key(path: tuple) -> str:
        if path[-1] in {"embedding", "kernel"}:
            path = (*path[:-1], "weight")
        return "model." + ".".join(map(str, path))
...
def test_qwen3():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert np.allclose(hf_outputs.hidden_states[26], outputs["hidden_states"][26], rtol=1e-3, atol=1e-3)
